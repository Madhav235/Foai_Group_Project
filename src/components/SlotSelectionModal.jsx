    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-200 bg-slate-950 px-6 py-5">
          <div>
            <h2 className="text-2xl font-semibold text-white">Select Slot</h2>
            <p className="mt-1 text-sm text-slate-300">
              Choose a slot to view available rooms.
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="w-full max-w-md space-y-3">
            {slots.length ? (
              slots.map((slot, index) => {
                ...
                const slotTime =
                  slot.time ?? `${slot.start ?? ""} - ${slot.end ?? ""}`;

                // Apply global rule for subject display
                const subjects = slot.subjects || [
                  {
                    subject: slot.subject,
                    rooms: slot.rooms,
                    completeSeating: slot.completeSeating,
                  },
                ];
                const subjectDisplay =
                  subjects.length > 1
                    ? "Multiple Subjects"
                    : (subjects[0].subject ?? "No Subject");
              ...
              <div className="rounded-[20px] border border-slate-200 bg-slate-50 px-8 py-12 text-center text-slate-600">
                ...
                <p className="mt-2 text-sm">This day has no slot data.</p>
              </div>
            )}
          </div>
        </div>
